...
def write_ledger_rows_to_template(
    ...
    for row_offset, ledger_row in enumerate(ledger_rows):
        ...
        # 先显式清空收入、支出、余额三列，避免模板残留旧值。
        for amount_field in ["收入", "支出", "余额"]:
            amount_col_idx = col_map.get(amount_field)
            if amount_col_idx is not None:
                safe_set_cell_value(ws, excel_row, amount_col_idx, "")
        ...
        for field_name, value in ledger_row.items():
            ...
            # 关键约束：一条台账明细只能是收入或支出其中一个方向。
            # 收入行写收入列、清空支出列；支出行写支出列、清空收入列。
            if field_name == "收入" and normalize_amount_for_excel(value) > 0:
                expense_col_idx = col_map.get("支出")
                if expense_col_idx is not None:
                    safe_set_cell_value(ws, excel_row, expense_col_idx, "")
            if field_name == "支出" and normalize_amount_for_excel(value) > 0:
                income_col_idx = col_map.get("收入")
                if income_col_idx is not None:
                    safe_set_cell_value(ws, excel_row, income_col_idx, "")
